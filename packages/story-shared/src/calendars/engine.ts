...
export class CalendarEngine {
  ...
  private calculateSubdivisions(dayOfYear: number): Map<string, number> {
    ...
    for (const subdivision of this.config.subdivisions) {
      remainingDays = this.processSubdivision(
        subdivision,
        remainingDays,
        result
      );
   ...
   */
  formatDate(date: ParsedDate, includeTime: boolean = true): string {
    ...
    if (this.config.specialDays) {
      for (const [subdivisionId, specialDayMap] of Object.entries(
        this.config.specialDays
      )) {
        ...
        if (subdivisionValue !== undefined) {
          ...
          const dayInSubdivision = this.getDayOfSubdivision(
            date,
            subdivisionId
          );
    ...
    for (const [subdivisionId, value] of date.subdivisions) {
      ...
      if (template.includes(dayOfPlaceholder)) {
        ...
        if (!isActualSubdivision) {
          ...
          template = template.replace(
            new RegExp(dayOfPlaceholder.replace(/[{}]/g, '\\$&'), 'g'),
            dayInSub.toString()
          );
        }
      }
   ...
   */
  formatStoryTime(time: StoryTime, includeTime: boolean = true): string {
